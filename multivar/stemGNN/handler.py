...
def inference(model, dataloader, device, node_cnt, window_size, horizon):
    ...
    with torch.no_grad():
        for i, (inputs, target) in enumerate(dataloader):
            ...
            while step < horizon:
                forecast_result, a, backcast_result = model(inputs)
...
def validate(model, dataloader, device, normalize_method, statistic,
             ...
             result_file=None):
    start = datetime.now()
    forecast_norm, target_norm, _, _ = inference(model, dataloader, device,
                                           node_cnt, window_size, horizon)
...
def train(train_data, valid_data, args, result_file):
    ...
    model = Model(node_cnt, args.window_size, args.multi_layer, horizon=args.horizon, stack_cnt=2)
    ...
    for epoch in range(args.epoch):
        ...
        for i, (inputs, target) in enumerate(train_loader):
            ...
            model.zero_grad()
            forecast, attn, backcast = model(inputs)
            fc_loss = forecast_loss(forecast, target)
            if args.use_backcast_loss:
                bc_loss = forecast_loss(backcast.squeeze(1).permute(0, 2, 1), inputs)
