...
class UltimateDataset(Dataset):
    def __init__(self, series_vals, series_ts, config):
        ...
        self.Normalizer = self.config.normalization_method(temporal_axis=1)
        ...
        self.full_norm_params = \
            self.Normalizer._extract_parameters(np.expand_dims(self.values, 0),
                                                temporal_axis=1)
    ...
    def _get_batch(self, batch_ix):
        ...
        '''
        if batch_ix > self.__len__()-1:
            return None
        start_ix = batch_ix * self.batch
        end_ix = start_ix + self.batch
    ...
    def __getitem__(self, index):
        # get raw batch
        '''
        inp_vals: R[batch x temporal x num_variates]
        tgt_vals: R[batch x temporal x num_variates]
        inp_covs: R[batch x temporal x num_covariates]
        tgt_covs: R[batch x temporal x num_covariates]
        '''
        inp_vals, tgt_vals, inp_covs, tgt_covs = self._get_batch(index)
        ...
        inputs = np.concatenate([inp_vals_norm, inp_covs], axis=2)
        targets = np.concatenate([tgt_vals_norm, tgt_covs], axis=2)
...
s = time()
for batch_ix in range(len(uds)):
    batch = next(iter(uds))
e = time()
print('batching in dataset.', e-s)
print(batch[0].shape)
